"""Asynchronous client for the RedScrape proxy API."""
...
class AsyncProxyClient:
    """Asynchronous client for fetching proxies from RedScrape API."""
    ...
    def __init__(self, 
        ...
        """
        Initialize the async proxy client.
    ...
    async def get_proxies(self, 
        ...
        """
        Fetch proxies from the API asynchronously.
        ...
        try:
            ...
            async with self.session.get(url, params=params) as response:
                ...
                if 'application/json' in content_type:
                    ...
                    return self._parse_json_response(data, filters)
                else:
                    ...
                    if raw_response:
                        ...
                        return self._parse_text_response(text, filters)
                        
        except asyncio.TimeoutError:
            raise ProxyTimeoutError(f"Request timed out after {self.timeout.total} seconds")
    ...
    async def get_proxy_urls(self, filters: Optional[Union[ProxyFilter, dict]] = None) -> List[str]:
        """
        ...
        Returns:
            List of proxy URLs
        """
        response = await self.get_proxies(filters)
        return [proxy.url for proxy in response.proxies]
    
    async def get_proxy_dicts(self, filters: Optional[Union[ProxyFilter, dict]] = None) -> List[dict]:
        """
    ...
    async def get_working_proxies(self, filters: Optional[Union[ProxyFilter, dict]] = None) -> ProxyResponse:
        """
    ...
    async def get_proxies_by_country(self, country_code: str, 
    ...
    async def get_multiple_countries(self, country_codes: List[str], 
                                   filters: Optional[Union[ProxyFilter, dict]] = None) -> List[ProxyResponse]:
        """
        Get proxies from multiple countries concurrently.
        ...
        Returns:
            List of ProxyResponse objects for each country
        """
        tasks = []
        for country_code in country_codes:
            task = self.get_proxies_by_country(country_code, filters)
            tasks.append(task)
